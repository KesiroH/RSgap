...
from .joint_names import ROBOT_JOINT_NAME_DICT_URDF_FOURIOR
...
class MotionLoaderMotor:
    ...
    def __init__(self, motion_file: str, device: torch.device | str, mode: str, robot_name: str) -> None:
        ...
        Args:
            ...
            mode: Mode of operation ('train' or 'play').
        ...
        # For Fourier robots, use URDF joint names from local joint_names.py
        self._dof_names = ROBOT_JOINT_NAME_DICT_URDF_FOURIOR[f"{robot_name}_joints"]
        try:
            self._body_names = ROBOT_JOINT_NAME_DICT_URDF_FOURIOR[f"{robot_name}_links"]
        except KeyError:
            self._body_names = []
        ...
        if "hand_marker" in data:
            self.hand_marker = torch.from_numpy(data["hand_marker"]).long().to(self.device)
        else:
    ...
    def sample_indices(self, num_samples: int, randomize_start: bool = False, min_available_length: int = 1) -> tuple[torch.Tensor, torch.Tensor]:
